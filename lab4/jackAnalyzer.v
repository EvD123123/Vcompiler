...
// CompilationEngine - handles syntax analysis
...
fn (mut c CompilationEngine) compile_subroutine_call() {
	...
	c.write_terminal() // subroutineName or className/varName

	if c.tokenizer.current_token() == '.' {
		c.write_terminal() // '.'
		c.write_terminal() // subroutineName
	}
...
fn (mut ja JackAnalyzer) analyze_file(file_path string) ! {
	...
	// Stage 1: Tokenizer output (XxxT.xml)
	base_name := file_path.replace('.jack', '')
	token_output_path := base_name + 'T.xml'
	ja.generate_token_output(input, token_output_path)!

	// Stage 2 & 3: Parser output (Xxx.xml)
	parse_output_path := base_name + '.xml'
	ja.generate_parse_output(input, parse_output_path)!
}

fn (mut ja JackAnalyzer) generate_token_output(input string, output_path string) ! {
	mut tokenizer := new_jack_tokenizer(input)
	mut output := strings.new_builder(1000)

	output.writeln('<tokens>')

	for tokenizer.has_more_tokens() {
		token_type := tokenizer.token_type()
		token_value := tokenizer.current_token()

		match token_type {
			.keyword {
				output.writeln('<keyword> ${token_value} </keyword>')
			}
			.symbol {
				// Handle XML escape sequences
				escaped_value := match token_value {
				...
				output.writeln('<symbol> ${escaped_value} </symbol>')
			}
			.identifier {
				output.writeln('<identifier> ${token_value} </identifier>')
			}
			.int_const {
				output.writeln('<integerConstant> ${token_value} </integerConstant>')
			}
			.string_const {
				output.writeln('<stringConstant> ${token_value} </stringConstant>')
			}
		}
	...
	os.write_file(output_path, output.str())!
	println('Generated token output: ${output_path}')
}

fn (mut ja JackAnalyzer) generate_parse_output(input string, output_path string) ! {
	mut tokenizer := new_jack_tokenizer(input)
	...
	println('Generated parse output: ${output_path}')
}
